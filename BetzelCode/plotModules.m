    ...
    for ii = 1:length(c) % for each recording...
        ...
        for jj = 1:length(n)
            ...
            for kk = 1:size(proc.raster,3)
                patch(spatialInfo.ROIs{kk}(:,2),spatialInfo.ROIs{kk}(:,1),c(data(mm).communities{comInd}(kk,jj),:),'facealpha',0.5,'edgealpha',1);
                if ismember(kk,sigCells)
                    plot(spatialInfo.ROIs{kk}(:,2),spatialInfo.ROIs{kk}(:,1),'w','LineWidth',2)
                    [~, bf] = max(mresp(kk,:));
                    bf = uT(bf);
                    text(spatialInfo.centroid(kk,1),spatialInfo.centroid(kk,2)-12,sprintf('%0.1f',round(bf/1000,1)),'Color','w','FontSize',8)
                end
            end
