...
%              return
            ...
            if ~isfield(netanal_tone,'sig_bfDistribution')
                ...
                % in each module?
                nBfsMod = cell(1,size(netanal_tone.conCom,2));
                ...
                nShuf=2000;
                nBfsMod_shuf = cell(1,size(netanal_tone.conCom,2));
                for ss = 1:nShuf
                ...
                sig_bfDistribution = cell(1,size(netanal_tone.conCom,2));
                for jj = 1:size(netanal_tone.conCom,2)
                    uTunedCellModules{jj} = unique(tunedCellModules(:,jj));
                    for nn = 1:length(uTunedCellModules{jj})
                        for kk = 1:length(uTunedBfs)
                ...
                netanal_tone.sig_bfDistribution = sig_bfDistribution;
