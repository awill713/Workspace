...
saveDir = fullfile('E:\Electrophysiology','EP004','MetaData - AVMultiContrastDriftingGratingsWhiteNoise','D prime - individual neuron');
if ~exist(saveDir)
    mkdir(saveDir);
end
% 
...
for dp = 1:length(dataPaths)
    ...
    if ~exist('dprime','var')
        ...
        highVsLow = cell(1,2);
        peakDPrime = cell(1,length(contrasts)); %going to be [vPeak avPeak diffPeak dp u neuronNumber]
        meanDPrime = cell(1,length(contrasts));
        orthoDPrime = cell(1,length(contrasts));
        oppDPrime = cell(1,length(contrasts));
    end
...
%                     
                    ...
                    vPeak = max(alignedV(targetIndex,:));
                    avPeak = max(alignedAV(targetIndex,:));
                    vMean = mean(alignedV(targetIndex,[1:6 8:13]));
                    avMean = mean(alignedAV(targetIndex,[1:6 8:13]));
                    vOrtho = mean(alignedV(targetIndex,[4 10]));
                    avOrtho = mean(alignedAV(targetIndex,[4 10]));
                    vOpp = alignedV(targetIndex,1);
                    avOpp = alignedAV(targetIndex,1);
                    diffPeak = avPeak - vPeak;
                    diffMean = avMean - vMean;
                    diffOrtho = avOrtho - vOrtho;
                    diffOpp = vOpp - avOpp;
                    peakDPrime{c} = [peakDPrime{c}; vPeak avPeak diffPeak dp u neuronNumber];
                    meanDPrime{c} = [meanDPrime{c}; vMean avMean diffMean dp u neuronNumber];
                    orthoDPrime{c} = [orthoDPrime{c}; vOrtho avOrtho diffOrtho dp u neuronNumber];
                    oppDPrime{c} = [oppDPrime{c}; vOpp avOpp diffOpp dp u neuronNumber];
...
orthoDiff = zeros(length(contrasts),size(highPerformance{c,1},1));
oppositeDiff = zeros(length(contrasts),size(highPerformance{c,1},1));
f4 = figure;
...
for c = 1:length(contrasts)
    ...
    diffData(c,:) = mean(highPerformance{c,2}(:,[1:6, 8:13]) - highPerformance{c,1}(:,[1:6, 8:13]),2)';
    orthoDiff(c,:) = mean(highPerformance{c,2}(:,[4 10]) - highPerformance{c,1}(:,[4 10]),2)';
    oppositeDiff(c,:) = highPerformance{c,2}(:,1) - highPerformance{c,1}(:,1);
    ...
    hA(2).FaceAlpha = 0.5;
    hA(2).EdgeColor = [1 1 1];
    plot(-180:30:180,nanmean(highPerformance{c,1}),'Color',[0 0 0]);
    plot(-180:30:180,nanmean(highPerformance{c,2}),'Color',[0 0 1]);
    ylim([0 1.2]);
    xticks(-180:90:180);
    xlabel('\Delta orientation');
    ylabel('D prime');
    ...
    hA = area(-180:30:180,[dd-ddSTD; 2*ddSTD]');
    hA(1).FaceAlpha = 0;
    hA(1).EdgeColor = [1 1 1];
    hA(2).FaceColor = [0 0 0];
    hA(2).FaceAlpha = 0.5;
    hA(2).EdgeColor = [1 1 1];
    plot(-180:30:180,dd,'Color',[0 0 0]);
    ylim([-0.3 0.3]);
    xticks(-180:90:180);
    xlabel('\Delta orientation');
...
%     ylim([-0.3 0.3]);
    ...
    figure;hold on;
    ddOrtho = mean(orthoDiff,2)';
    ddOrthoSTD = std(orthoDiff,[],2)'/sqrt(size(orthoDiff,2));
    
    hA = area(contrasts,[ddOrtho-ddOrthoSTD; 2*ddOrthoSTD]');
    hA(1).FaceAlpha = 0;
    hA(1).EdgeColor = [1 1 1];
    hA(2).FaceColor = [0 0 0];
    hA(2).FaceAlpha = 0.5;
    hA(2).EdgeColor = [1 1 1];
    plot(contrasts,ddOrtho,'Color',[0 0 0]);
%     ylim([-0.3 0.3]);
    xticks(contrasts);
    xlabel('Contrast');
    ylabel('\Delta D prime ortho');
       ...
       figure;hold on;
    ddOpp = mean(oppositeDiff,2)';
    ddOppSTD = std(oppositeDiff,[],2)'/sqrt(size(oppositeDiff,2));
    
    hA = area(contrasts,[ddOpp-ddOppSTD; 2*ddOppSTD]');
    hA(1).FaceAlpha = 0;
    hA(1).EdgeColor = [1 1 1];
    hA(2).FaceColor = [0 0 0];
    hA(2).FaceAlpha = 0.5;
    hA(2).EdgeColor = [1 1 1];
    plot(contrasts,ddOpp,'Color',[0 0 0]);
%     ylim([-0.3 0.3]);
    xticks(contrasts);
    xlabel('Contrast');
...
% save(fullfile(saveDir,'dPrimeData.mat'),'dprime','peakDPrime','meanDPrime','orthoDPrime','oppDPrime');