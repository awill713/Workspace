...
newDir = fullfile('E:\Electrophysiology\',experiment,mouseID,date,'AVMultiContrastDriftingGratingsWhiteNoise');
...
for n = 1:totalUnits
    ...
    p = anovan(meanResponse(:,4),{stimInfo.index(:,2),stimInfo.index(:,3),stimInfo.index(:,4)},'mode','interaction','varnames',{'orientation','contrast','sound'},'display','off');
    ...
    suptitle({['Unit ' num2str(nData.CellInfo(4)) ' (n = ' num2str(n)...
        '), unitType = ' num2str(nData.CellInfo(6))] ['Light responsive = ' num2str(p(2))...
        ', Sound responsive = ' num2str(p(3)) ', Orientation selective = ' num2str(p(1))...
        ', Sound-modulated = ' num2str(p(5)) ', ' num2str(p(6))]});saveas(f2,fullfile(figureDir,['Unit ' num2str(nData.CellInfo(4)) ' response polar plots.fig']));
...
%         subplot(1,length(contrasts),c); hold on;
    ...
    unitData(n).fanoFit = [pVis pVisAud];
