...
session = 'Session1';
date = '20200919';
stimPath = fullfile('D:\Electrophysiology\',experiment,mouseID,date,'StimInfo',[date '_' mouseID '_AVsDriftingGratingsWhiteNoise_blueLaser_stimInfo']);
...
newDir = fullfile('D:\Electrophysiology\',experiment,mouseID,date,'AVOptoDriftingGratingsWhiteNoiseBlueLaser');
...
for n = 1:totalUnits
    ...
    for c = 1:length(contrasts)
        ...
        subplot(4,length(contrasts),c+2*length(contrasts));hold on;
        for r = 1:length(orientations)
            evIdx = r + (c-1)*length(orientations) + length(orientations)*length(contrasts)*2;
            scatter(spikeRaster{evIdx,1},spikeRaster{evIdx,2},[],spikeRaster{evIdx,3},'.');
            hold on;
        end
        xlim([analysisWindow]);
        title(['Cont = ' num2str(contrasts(c)) ', light+laser']);
        ...
        xlim([analysisWindow]);
        title(['Cont = ' num2str(contrasts(c)) ', light+sound+laser']);
...
%         xlabel('Time (ms)');
    ...
    for c = 1:length(contrasts)
        ...
        evIdx = 1 + (c-1)*length(orientations);
        idxOffset = length(orientations)*length(contrasts);
        laserOffset = length(orientations)*length(contrasts)*2;
        bothOffset = length(orientations)*length(contrasts)*3;
        ...
        polarplot(2*pi*orientations./360,meanResponse((evIdx+laserOffset):(evIdx+laserOffset+length(orientations)-1),4),'Color',[0 0 0],'LineWidth',2,'LineStyle',':');
        polarplot(2*pi*orientations./360,meanResponse((evIdx+bothOffset):(evIdx+bothOffset+length(orientations)-1),4),'Color',[0 0 1],'LineWidth',2,'LineStyle',':');
...
for n = 1:totalUnits
    if unitData(n).type~=0
        if ismember(unitData(n).neuronNumber,responsiveUnits.soundAndLight)
            lightR = mean(unitData(n).meanResponse(13:24,4));
            soundR = mean(unitData(n).meanResponse(37:48,4));
            laserR = mean(unitData(n).meanResponse(61:72,4));
            bothR = mean(unitData(n).meanResponse(85:96,4));
            
            lsl = [lsl; lightR soundR laserR bothR];
        end
    end
end
f3 = figure;
...
ylabel('Light + Laser response (Hz)');
limX = xlim; limY = ylim;
...
scatter(lsl(:,2),lsl(:,4));
xlabel('Light + Sound response (Hz)');
ylabel('Light + Sound + Laser response (Hz)');
line([0 max([limX(2) limY(2)])],[0 max([limX(2) limY(2)])]);
saveas(f3,fullfile(newDir,['Light Sound Laser comparison scatter plot.fig']));
saveas(f3,fullfile(newDir,['Light Sound Laser comparison scatter plot.jpg']));
