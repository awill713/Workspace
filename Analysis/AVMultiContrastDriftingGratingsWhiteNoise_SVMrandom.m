...
%                     [c nc rando iDir jDir]
                    ...
                    for trial = 1:repeats
                        ...
                        trainingVec = zeros(length(iTrainingTrials)+length(jTrainingTrials),nc,1);
                        testingVec = zeros(2,nc);
                        for neuron = 1:nc
                            chosenUnit = randoUnits(neuron);
                            iDataV = masterLog(chosenUnit).trialResponse(vInd+iDir,iTrainingTrials);
                            jDataV = masterLog(chosenUnit).trialResponse(vInd+jDir,jTrainingTrials);
                            iDataAV = masterLog(chosenUnit).trialResponse(avInd+iDir,iTrainingTrials);
                            jDataAV = masterLog(chosenUnit).trialResponse(avInd+jDir,jTrainingTrials);
                            
                            trainingVec(:,neuron,1) = [iDataV'; jDataV'];
                            testingVec(1,neuron) = masterLog(chosenUnit).trialResponse(vInd+iDir,iTestTrials);
                            
                            trainingVec(:,neuron,2) = [iDataAV'; jDataAV'];
                            testingVec(2,neuron) = masterLog(chosenUnit).trialResponse(avInd+iDir,iTestTrials);
                        end
                        
%                         svmVis = fitcsvm(squeeze(trainingVec(:,:,1)),trainingAnswers,'KernelFunction','Linear','Standardize',true,'KernelScale','auto');
                        svmVis = fitcsvm(squeeze(trainingVec(:,:,1)),trainingAnswers,'KernelFunction','Linear','Standardize',true);
                        testPrediction = svmVis.predict(testingVec(1,:));
%                         accuracy(1,trial) = mean(testPrediction==1); %1 is iDir answer
                        accuracy(1,trial) = testPrediction==1; %1 is iDir answer
                        
%                         svmAudVis = fitcsvm(squeeze(trainingVec(:,:,2)),trainingAnswers,'KernelFunction','Linear','Standardize',true,'KernelScale','auto');
                        svmAudVis = fitcsvm(squeeze(trainingVec(:,:,2)),trainingAnswers,'KernelFunction','Linear','Standardize',true);
                        testPrediction = svmAudVis.predict(testingVec(2,:));
%                         accuracy(2,trial) = mean(testPrediction==1); %1 is iDir answer
                        accuracy(2,trial) = testPrediction==1; %1 is iDir answer
                    end
