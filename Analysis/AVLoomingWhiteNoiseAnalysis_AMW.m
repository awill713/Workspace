...
stimPath = fullfile('D:\Electrophysiology\',experiment,mouseID,date,'StimInfo',[date '_' mouseID '_AVloomingWhiteNoise_exptInfo']);
...
newDir = fullfile('D:\Electrophysiology\',experiment,mouseID,date,'AVLoomingWhiteNoiseResponses');
...
load(stimPath);
uniqueEvents = size(exptInfo.index,1);
indices = exptInfo.index(:,1);
...
for n = 1:totalUnits
    ...
    trialResponses = zeros(uniqueEvents,exptInfo.repeats);
    frTrain = zeros(uniqueEvents,binCount);
    
    for u = 1:uniqueEvents
        eventID = indices(u);
        eventsOfInterest = find(exptInfo.order==eventID);
...
%     
    ...
    suptitle(['Unit ' num2str(nData.CellInfo(4)) ' (n = ' num2str(n) '), Raster Plot']);
    ...
    for modality1=1:4
        for modality2 = 1:4
            ...
            audInd = find(exptInfo.index(:,2)==modality1);
            visInd = find(exptInfo.index(:,3)==modality2);
            ...
            audInd = find(exptInfo.index(:,2)==modality2);
            visInd = find(exptInfo.index(:,3)==modality1);
            ind = intersect(audInd,visInd);
    ...
    suptitle(['Unit ' num2str(nData.CellInfo(4)) ' (n = ' num2str(n) '), Firing Rate']);
    ...
    ylabel('Sound');
    colorbar
    title(['Unit ' num2str(nData.CellInfo(4)) ' (n = ' num2str(n) '), Response Heat Map']);
